...
function BigMultiplication(firstNum: number, secondNum: number){
    const firstNumString = firstNum.toString();
    const secondNumString = secondNum.toString();
    

    console.log(
        `+${PrintOut('-', (firstNumString.length+3+(firstNumString.length*3)))}+
        |   ${}   |
    `);
...
function PrintOut(character: string, length:number){
    let value: string = "";
